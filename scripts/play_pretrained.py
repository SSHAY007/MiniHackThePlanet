...
def load_model(env, pretrained_path, pretrained_config_path, device):
    ...
    #checkpoint_states = torch.load(torch.load(pretrained_path),strict=False)

    #logging.info(checkpoint_states)
    print(checkpoint_states["model_state_dict"])
...
def main():
    ...
    parser.add_argument(
        ...
        "--env",
        type=str,
        default="MiniHack-River-Lava-v0",
        #default="MiniHack-SimpleSeq-v0",
        help="Gym environment spec. Defaults to 'NetHackStaircase-v0'.",
    )
    parser.add_argument(
        "--agent_env",
        type=str,
        default="mini_skill_nav_lava",
    ...
    parser.add_argument(
        "--pretrained_path",
        type=str,
        #default="outputs/2023-10-29/13-04-30/mini_simple_seq_0.tar",
        default="skill_transfer_weights/mini_skill_nav_lava.tar",
        help="Path to checkpoint to load pretrained model.",
    )
    parser.add_argument(
        "--pretrained_config_path",
        type=str,
        default="skill_transfer_weights/skill_config.yaml",
